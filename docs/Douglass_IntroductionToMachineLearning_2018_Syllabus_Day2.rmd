...
NN = neuralnet(formula_onehot,
               data= BreastCancer_onehot_train %>% data.matrix(), 
               hidden = 10 , 
               linear.output = T )
...
```{r, eval=F, include=F}
#This fails because caret doesn't handle classification with neuralnet
...
test_class_cv_model <- train(x=BreastCancer_onehot_train[,-c(6)] %>% data.matrix(),
                             y=as.factor(paste0('Outcome',BreastCancer_train$y)),
                             method = "neuralnet",
                             trControl = cctrl1,
                             metric = "ROC",
                             tuneGrid =expand.grid(linear.output = T) ,
